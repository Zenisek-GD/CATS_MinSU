...
class SimulationEnhancedSeeder extends Seeder
{
    ...
    private function seedJobPhishing(): void
    {
        ...
        $s3 = SimulationStep::query()->create(['simulation_id'=>$sim->id,'step_order'=>3,'title'=>'Protecting Your Identity','prompt'=>"What's the best way to handle this situation now?",'education'=>'If you shared info with a scammer: monitor your bank/GCash, change passwords, contact your bank, file a report with NBI Cyber.']);
        ...
        SimulationChoice::query()->create(['step_id'=>$s3->id,'next_step_id'=>null,'text'=>'Report to Authorities: Report the scam to MinSU IT and NBI Cyber','is_safe'=>true,'score_delta'=>25,'feedback'=>'Excellent! Reporting helps investigate and shut down scam operations.','explanation'=>'File a report with NBI Cyber Division. Also notify MinSU IT to warn other students.','sort_order'=>1]);
