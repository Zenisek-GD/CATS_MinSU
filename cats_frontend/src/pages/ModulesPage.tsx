...
const CATEGORY_KEYWORDS: { label: string; keywords: string[] }[] = [
  ...
  { label: 'Cyber',   keywords: ['cyber', 'malware', 'ransomware', 'hacking', 'cyber'] },
...
export default function ModulesPage() {
  ...
  const [pretestDone, setPretestDone] = useState<boolean | null>(null) // null = loading
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        const [r, quizzesRes, attemptsRes] = await Promise.all([
          getModules(),
          getQuizzes({ kind: 'pretest' }),
          getMyQuizAttempts(),
        ])
        setModules(r.modules)
        // Lock Learn if there are pretests and none have been attempted
        const pretests = quizzesRes.quizzes.filter(q => q.kind === 'pretest')
        if (pretests.length === 0) {
          setPretestDone(true) // No pretests exist — allow access
        } else {
          const done = pretests.some(q => !!attemptsRes[String(q.id)])
          setPretestDone(done)
        }
      }
      ...
      <div className="modulesShell">
        ...
        <aside className="modulesSidebar" aria-label="Sidebar navigation">
          ...
          <nav className="modulesNav">
            {pretestDone === false ? (
              <div className="modulesNavItem modulesNavItem--locked" aria-label="Learn (locked)" title="Complete the Pre-Test first">
                <Icon name="lock" size={20} /><span>Learn</span>
                <span className="modulesNavLockBadge">Locked</span>
              </div>
            ) : (
        ...
        <div className="modulesMain">
          ...
          <main className="modulesContent">
            {/* ── Pre-test Lock Gate ── */}
            {pretestDone === false && (
              <div className="mLockGate" role="alert" aria-label="Learn module locked">
                <div className="mLockGateInner">
                  <div className="mLockGateIcon">
                    <Icon name="lock" size={48} />
                  </div>
                  <h2 className="mLockGateTitle">Learn Module is Locked</h2>
                  <p className="mLockGateText">
                    You must complete the <strong>Pre-Test</strong> in the Assess section before
                    you can access the learning modules.
                  </p>
                  <Link to="/quizzes" className="mLockGateBtn">
            ...
            {/* ── Progress Hero ── */}
            {pretestDone !== false && <section className="modulesHero" aria-label="Overall progress">
            ...
            {pretestDone !== false && <div className="mControlsRow" aria-label="Search and filter">
              {/* Inline search */}
            ...
            {busy && modules.length === 0 && pretestDone !== false && <div className="modulesLoading">Loading modules…</div>}
            {!busy && filtered.length === 0 && pretestDone !== false && <div className="modulesEmpty">No modules match.</div>}

            {/* ── "Up Next" Hero Card ── */}
            {pretestDone !== false && heroModule && (() => {
            ...
            {/* ── Module Grid ── */}
            {pretestDone !== false && restModules.length > 0 && (
          ...
          <nav className="modulesBottomNav" aria-label="Bottom navigation">
            {pretestDone === false ? (
              <span className="bottomNavItem bottomNavItem--locked" aria-label="Learn locked" title="Complete Pre-Test first">
                <Icon name="lock" size={20} /><span>Learn</span>
              </span>
            ) : (
